...
interface FavoritesDistributionProps {
  favorites: ReadonlyArray<readonly [string, Pokemon[]]>;
  totalPokemon: number;
}
...
function FavoriteRow({ label, pokemon, totalPokemon }: FavoriteRowProps) {
...
export function FavoritesDistribution({
  favorites,
  totalPokemon,
}: FavoritesDistributionProps) {
  return (
    <>
      ...
      <Stack spacing={0.75}>
        {favorites.map(([favorite, pokemon]) => (
          <FavoriteRow
            key={favorite}
            label={favorite}
            pokemon={pokemon}
